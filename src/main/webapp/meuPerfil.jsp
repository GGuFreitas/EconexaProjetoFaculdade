<%-- 
    Document   : meuPerfil
    Descrição  : Exibe o perfil do usuário, suas interações e permite a edição de dados de login.
    Updated    : Validação de senha ajustada para mínimo de 6 caracteres.
    Author     : gufre / jhonny
--%>
...
<%
    ...
    // Redirecionamento de segurança: Se o usuário não estiver logado, envia para o login.
    if (usuario == null) {
        response.sendRedirect("login.jsp");
        return;
    }

    // Prepara dados para exibição (primeiro nome)
    String nomeExibicao = usuario.getNome().split(" ")[0];
    boolean estaLogado = true; // Variável de controle (opcional)
    
    // Instancia o DAO e carrega os posts para as abas de listagem
    ...
    // Captura mensagens de sucesso ou erro (vindas do EditarPerfilServlet)
    ...
    <body>
                 ...
                 <div class="container-fluid">
                    ...
                    <div class="collapse navbar-collapse" id="mainNavbar">
                        <ul class="navbar-nav nav-pills mb-2 mb-lg-0">
                            ...
                            <%-- A verificação isAdmin() foi mantida para exibir o link Admin --%>
                            <% if (usuario.isAdmin()) { %> 
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', () => {
                // Tenta manter a aba 'edicao' ativa se houver erro ou sucesso no parâmetro URL
                const urlParams = new URLSearchParams(window.location.search);
                const isStatusMessage = urlParams.has('sucesso') || urlParams.has('erro');
                
                if (isStatusMessage) {
                     // Se houve uma mensagem, abre a aba de edição para que o usuário veja a notificação
                    openTab('edicao', document.getElementById('btn-edicao'));
                } else {
                    // Senão, abre a aba padrão (Minhas Publicações)
                    openTab('meus', document.getElementById('btn-meus'));
                }
            });

            /**
             * Alterna o conteúdo visível entre as abas do perfil.
             */
            ...
            /**
             * Prepara e exibe o sidebar de edição de um post específico.
             */
            ...
            /**
             * Realiza a validação das senhas e a confirmação do usuário antes de submeter
             * o formulário de edição de perfil.
             * @param {Event} event O evento de submissão do formulário.
             * @returns {boolean} True para prosseguir com a submissão, false para cancelar.
             */
            function validarFormulario(event) {
                ...
                if (novaSenha || confirmaSenha) {
                    // 1. Confere se as senhas coincidem
                    if (novaSenha !== confirmaSenha) {
                        alert('Erro: A nova senha e a confirmação de senha não coincidem. Por favor, corrija.');
                        event.preventDefault(); // Impede o envio
                        return false;
                    }
                    // 2. Confere o comprimento mínimo (ajustado para 6, conforme atributo minlength)
                ...
                // 3. Confirmação final antes de alterar dados sensíveis
                // Esta confirmação é obrigatória, mesmo que a senha não tenha sido alterada.
